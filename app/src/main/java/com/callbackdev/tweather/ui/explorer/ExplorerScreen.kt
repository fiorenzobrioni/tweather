...
fun ExplorerScreen(
    ...
    Surface(modifier = Modifier.fillMaxSize(), color = MaterialTheme.colorScheme.background) {
        Column(Modifier.fillMaxSize()) {
            // The one tab that isn't a file: it shows the folder it browses, with the
            // trailing slash a path listing would have — "explorer" named the pane,
            // not the content, and was the only tab title outside the metaphor.
            EditorTab(fileName = "cities/")
            Column(
                Modifier
                    .weight(1f)
